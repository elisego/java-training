...
public class ArrayTraining {
    ...
    public int[] reverse(int[] array) {
        int[] arrayBis = new int [array.length];
        for (int i = 0; i < array.length; i ++){
            arrayBis[i] = array[array.length - i - 1 ];
        }
        return arrayBis;
    }
    ...
    public int[] concat(int[] first, int[] second) {
        int [] arrayBis = new int [first.length + second.length];
        for ( int i = 0; i < (first.length); i ++){
            arrayBis[i]= first[i];
        }
        for ( int i = 0; i< arrayBis.length -first.length; i ++){
            arrayBis[i - first.length] = second[i];
        }
        return arrayBis;
    }
}
